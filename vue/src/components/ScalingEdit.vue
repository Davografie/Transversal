<script setup lang="ts">
	...
	const { y } = useScroll(number_wrapper)
	...
	watch(selected, (newSelected) => {
		emit('change-scaling', newSelected)
	})

	watch(new_scaling, () => {
		// scroll to selected scaling
		scroll_to_selected()
		emit('change-scaling', new_scaling.value)
	})
...
<template>
	<div class="scaling-edit">
		...
		<div class="number-wrapper" ref="number_wrapper">
			<span class="scale-number" v-for="n of number_array" :key="n" @click="emit('change-scaling', n)" :class="{ 'selected': n == new_scaling }">
				{{ n }}
			</span>
		</div>
		<input value="▼" type="button" class="button-mnml" @click="change_scaling(-1)" />
